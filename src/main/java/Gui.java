...
public class Gui extends Canvas {
    ...
    private static final double rot = Math.toRadians(-30.0);
    private Cave c;
    ...
    public void renderCave(int xOffset, int yOffset, int size){
        for(Cave.Cell C:c.getCavern()){
            ...
            for (int i = 0; i < 6; i++) p.addPoint(r(x+(size-1)*Math.cos(i*e+rot)),r(y+(size-1)*Math.sin(i*e+rot)));
            ...
            for(int j=0;j<6;j++){
                ...
                g2.drawLine(
                    r(x+(size-2)*Math.cos(j*e+rot)),
                    r(y+(size-2)*Math.sin(j*e+rot)), 
                    r(x+(size-2)*Math.cos((j+1)*e+rot)), 
                    r(y+(size-2)*Math.sin((j+1)*e+rot))
                );
            }
            g2.setColor(Color.BLACK);
